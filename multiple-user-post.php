...
function multiple_user_post_markup_admin($object) {
    ...
    <div>
        <div class="ui-widget">
            <label for="birds"><?php _e("Assign Users To this post: ");?></label>
            <input id="birds">
        </div>
        ...
        <?php if ($users) { ?>
            <div>
                <b><?php _e("Already Assigned Users:"); ?></b>
                <?php
                foreach ($users as $user) {
                    $user_info = get_user_by('id', $user);
                    echo "<div>".__($user_info->display_name)."</div>";
                }
                ?>
            </div>
        <?php } ?>
...
function multiple_user_post_save_admin($post_id, $post, $update) {
    ...
    if (isset($_POST["meta-box-user-store"])) {
        ...
        foreach ($users as $user_id_new) {
            $title = __(get_the_title($post_id) . "--Copy For User--" . $user_id_new);
            // Create post object
            $my_post = array(
                'post_title' => $title,
...
// Insert the post into the database
            if (!get_page_by_title($title, 'OBJECT', 'post')) {
                ...
                $subj = __('The Following Post Needs Your Suggestions');
                $body = __('Please click on below link to suggest edits: ') . get_edit_post_link($post_author_info[$user_id_new]);
...
function multiple_user_post_add_meta_admin() {
    add_meta_box("multiple-user-post-meta-box", __("Multiple User Post Meta Box"), "multiple_user_post_markup_admin", "post", "side", "high", null);
    add_meta_box("multiple-user-meta-box-content", __("User Content Sent For Approval"), "multiple_user_post_meta_box_content_admin", "post", "normal", "high", null);
}
...
function multiple_user_post_meta_box_content_admin($object) {
    ...
    if ($users_posts) {
        foreach ($users_posts as $user => $post_id) {
            $user_info = get_user_by('id', $user);
            ?>
            <h3><?php _e("Inputs From User:"); ?> <i><?php echo __($user_info->display_name); ?></i> </h3>
            <?php
